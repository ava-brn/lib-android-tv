...
export class RemoteManager extends Client {
    ...
    async connect(): Promise<boolean> {
        if (this.listenerCount('raw') < 1) {
            this.on('raw', (buffer) => this.handleRawMessage(buffer));
        }

        this.socket?.once('timeout', () => {
            this.emit('log', `Connection to [${this.host}] timed out.`);
            this.socket?.destroy();
        });
    ...
    handleRawMessage(buffer: Buffer) {
